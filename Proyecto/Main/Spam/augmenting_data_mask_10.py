...
def createDataAugmentationDictionaryGood(sentences, labels):
    ...
    for sentence, label in zip(sentences, labels):
        ...
        if len(tokenized_sentence) > 1:
            ...
            top_10_tokens = torch.topk(mask_token_logits, 3, dim = 1).indices[0].tolist()
...
print(sentences)
#   Create five new sentences for each sentence in the data
data_augmentation_dictionary = createDataAugmentationDictionaryGood(sentences, labels)
