...
def importData():
    filepath_dict = {'spam': 'NewSpam.csv'}
    data_file_list = []
...
data_file = pd.read_csv('spam.csv',delimiter=',',encoding='latin-1')
...
#   Mix the data
mixed_complete_sentences_list =[]
mixed_complete_labels_list = []
random_list = random.sample(range(len(complete_sentences_list)), len(complete_labels_list))
for i in random_list:
    mixed_complete_sentences_list.append(complete_sentences_list[i])
    mixed_complete_labels_list.append(complete_labels_list[i])
