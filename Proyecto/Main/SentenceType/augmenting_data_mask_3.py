...
def createSentenceListOne(predicted_sentences, label):
    ...
    for sen in predicted_sentences:
        ...
        sen_list.append(sen)
    return sen_list
def createSentenceListThree(predicted_sentences, label):
    sen_list = []
    for i in range(3):
        sen = predicted_sentences[i]['sequence']
        sen = sen.replace('<s> ', '')
        sen = sen.replace('</s>', '')
        sen = sen.replace('<s>', '')
        sen = sen + "\t" + str(label)
...
def createDataAugmentationDictionary(sentences, labels):
    ...
    for sentence, label in zip(sentences, labels):
        ...
        if len(tokenized_sentence) > 1:
            ...
            sentences_list = createSentenceListThree(predicted_sentences, label)
